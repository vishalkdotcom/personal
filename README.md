...
- AI crawlers: `static/robots.txt` allows GPTBot, ChatGPT-User, ClaudeBot, PerplexityBot, Google-Extended, and DeepSeekBot. If the **vishalk.com** zone turns managed robots.txt / Content Signals back on, Cloudflare prepends `Disallow: /` for GPTBot, ClaudeBot, and Google-Extended. If Block AI bots / Super Bot Fight Mode is on without a Skip rule, those UAs 403. Dashboard: Security → Bots → **Disable robots.txt configuration** (and Content Signals); WAF Skip at the top for:

...
async function processAllImages() {
  ...
  try {
    const files = (await fs.readdir(inputDir)).filter((file) => /\.(jpe?g|png|webp)$/i.test(file));
    ...
    console.log(
      `📈 Results: ${totalStats.successful} generated, ${totalStats.skipped} skipped, ${totalStats.failed} failed`,
    );

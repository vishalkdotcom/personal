...
- Agent files: `dist/llms.txt`, `dist/sitemap.xml`, per-path `.md` siblings, crawler HTML + JSON-LD in stamped shells. `static/robots.txt` explicitly allows GPTBot, ChatGPT-User, ClaudeBot, PerplexityBot, Google-Extended, and DeepSeekBot. On the **vishalk.com** zone today, two dashboard layers still hide that: Cloudflare managed robots.txt injects `Disallow: /` for GPTBot, ClaudeBot, and Google-Extended, and WAF 403s GPTBot, ChatGPT-User, ClaudeBot, and PerplexityBot (`Attention Required! | Cloudflare`). `*.pages.dev` previews have neither. Disable managed robots.txt and skip Super Bot Fight Mode / Block AI bots for those UAs (README Deploy), or they never reach Pages.

...
- Agent files: `dist/llms.txt`, `dist/sitemap.xml`, per-path `.md` siblings, crawler HTML + JSON-LD in stamped shells. `static/robots.txt` explicitly allows GPTBot, ChatGPT-User, ClaudeBot, PerplexityBot, Google-Extended, and DeepSeekBot. The vishalk.com zone must keep managed robots.txt / Content Signals off and skip Super Bot Fight Mode / Block AI bots for those UAs (README Deploy). CI job **Allow AI crawlers** applies that via `CLOUDFLARE_API_TOKEN`, or verifies live apex when the token is missing.

...
- Agent files: `dist/llms.txt`, `dist/sitemap.xml`, per-path `.md` siblings, crawler HTML + JSON-LD in stamped shells. `static/robots.txt` explicitly allows GPTBot, ChatGPT-User, ClaudeBot, PerplexityBot, Google-Extended, and DeepSeekBot. Custom-domain WAF still 403s GPTBot, ChatGPT-User, ClaudeBot, and PerplexityBot today (`Attention Required! | Cloudflare`); `*.pages.dev` previews do not. Skip Super Bot Fight Mode / Block AI bots for those UAs (expression in README Deploy), or they never reach Pages.

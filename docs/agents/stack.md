...
- Agent files: `dist/llms.txt`, `dist/sitemap.xml`, per-path `.md` siblings, crawler HTML + JSON-LD in stamped shells. `static/robots.txt` explicitly allows GPTBot, ChatGPT-User, ClaudeBot, PerplexityBot, Google-Extended, and DeepSeekBot. On the **vishalk.com** zone today, two dashboard layers still hide that: Cloudflare managed robots.txt / Content Signals (`cf_robots_variant`) inject `Disallow: /` for GPTBot, ClaudeBot, and Google-Extended, and WAF 403s GPTBot, ChatGPT-User, ClaudeBot, and PerplexityBot (`Attention Required! | Cloudflare`). `*.pages.dev` previews have neither. Disable managed robots.txt (including Content Signals) and skip Super Bot Fight Mode / remaining custom WAF / Block AI bots for those UAs (README Deploy), or they never reach Pages.
